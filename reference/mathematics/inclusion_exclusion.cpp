...
void update(const auto &number, const auto &change, const auto &factors, auto &have, auto &elements) {
    elements += change;
...
int main() {
    ...
    while (t-- > 0) {
        ...
        if (stock[pos] == true)
            update(id[pos], -1, factors, have, elements);
