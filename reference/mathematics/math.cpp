...
std::vector<int> math::build_lowest_divisors(const int MAX_SIZE) {
...
std::vector<int> math::get_factors(const int N,
...
int math::count_divisors(const int N, const std::vector<int> &lowest_divisors) {
  std::unordered_map<int, int> frequency;
  for (const int factor : math::get_factors(N, lowest_divisors)) {
    frequency[factor] += 1;
  }
  int ans = 1;
...
std::vector<int> math::build_factorial(const int MAX_SIZE, const int MOD) {
...
int math::fast_exponentiate(int base, int power, const int MOD) {
  int ans = 1;
  while (power > 0) {
    if (power & 1) {
...
std::vector<int> math::build_inverse_factorial(const std::vector<int> &fact,
                                               const int MOD) {
...
int math::combination(const int set_size, const int subset_size, const int MOD,
                      const std::vector<int> &fat,
                      const std::vector<int> &fat_inv) {
